...
public abstract class PullToRefreshBase<T extends View> extends LinearLayout {
	...
	/**
	 * Returns whether the SmoothScroll is doing
	 * 
	 * @return true if current smooth scroll is doing
	 */
	public final boolean isSmoothScrolling(){
		return null != currentSmoothScrollRunnable && !currentSmoothScrollRunnable.isDone();
	}
	...
	public final void setOnPullToRefreshListener(OnPullToRefreshListener listener) {
	...
	public void setHeaderLabel(String pullLabel,String refreshingLable, String releaseLable) {
		if (null != headerLayout) {
			headerLayout.setPullLabel(pullLabel);
			headerLayout.setRefreshingLabel(refreshingLable);
			headerLayout.setReleaseLabel(releaseLable);
			
		}
	}
	public void setFooterLabel(String pullLabel,String refreshingLable, String releaseLable) {
		if (null != footerLayout) {
			footerLayout.setPullLabel(pullLabel);
			footerLayout.setRefreshingLabel(refreshingLable);
			footerLayout.setReleaseLabel(releaseLable);
	...
	public final void setHeaderBackground(int resid) {
		...
		if(footerLayout!=null) {
			footerLayout.setBackgroundResource(resid);
		}
	}
	
	public final void setHeaderTextColor(int resid) {
		if(headerLayout!=null)
			headerLayout.setTextColor(getResources().getColor(resid));
		if(footerLayout!=null)
			footerLayout.setTextColor(getResources().getColor(resid));
	}
				...
				if (state == RELEASE_TO_REFRESH && null != onRefreshListener) {
					...
					if(null!= mPullToRefreshListener){
						if(currentMode == MODE_PULL_UP_TO_REFRESH){
							mPullToRefreshListener.onPullUp();
						}else if (currentMode == MODE_PULL_DOWN_TO_REFRESH) {
							mPullToRefreshListener.onPullDwon();
						}
					}
	...
	private void init(Context context, AttributeSet attrs) {
		...
		String pullLabel = getResources().getString(R.string.readmode_pull_nextpage);
		String refreshingLabel = getResources().getString(R.string.readmode_refresh_nextpage);
		String releaseLabel = getResources().getString(R.string.readmode_release_nextpage);

		// Add Loading Views
		if (mode == MODE_PULL_DOWN_TO_REFRESH || mode == MODE_BOTH) {
			headerLayout = new PullToRefreshLoadingLayout(context,
		...
		if (mode == MODE_PULL_UP_TO_REFRESH || mode == MODE_BOTH) {
			footerLayout = new PullToRefreshLoadingLayout(context,
	...
	final class SmoothScrollRunnable implements Runnable {
		...
		public boolean isDone(){
			Log.i("mxnews", "scrollToY" + scrollToY + "\ncurrentY" + currentY);
			return !(continueRunning && scrollToY != currentY);
		}
